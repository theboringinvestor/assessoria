...
var TBI_PERFIL = (function () {
  ...
  /* Nom explícit d'una cartera: «Perfil · Estratègia».
     Si el nom ja comença pel perfil («Conservador diversificat») s'evita la
     redundància i queda «Conservador · Diversificat». */
  function _nomCartera(a) {
    var n = a.nom;
    if (n.toLowerCase().indexOf(a.perfil.toLowerCase()) === 0) {
      n = n.slice(a.perfil.length).replace(/^[\s·-]+/, "");
      n = n.charAt(0).toUpperCase() + n.slice(1);
    }
    return n ? (a.perfil + " · " + n) : a.perfil;
  }
  ...
  function arquetipsOnboarding() {
    return ARQUETIPS.map(function (a, i) {
      ...
      return {
        ...
        cartera: _nomCartera(a), cartDesc: a.sub, perfil: a.perfil,
